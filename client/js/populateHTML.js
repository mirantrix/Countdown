...
function setTeam({abbreviation, flag, status}) {
  ...
  teamStatus.className = `${status} team-status`;
...
function setMoreInfoDiv({stadium, city, tournament}) {
  const div = setTag('div');
...
function jumbotron(image) {
  ...
  const jumbotronImage = setTag('img');
  jumbotronImage.src = `${image}.gif`;
  jumbotron.appendChild(jumbotronImage);
}
